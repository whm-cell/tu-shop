...
// 获取原生的页面实例
const currentPagesInstance = pages.at(-1) as any

// 实现动画效果，必须要等页面渲染完毕   相当于vue的mounted
onReady(() => {
  currentPagesInstance.animate?.(
    '.navbar', // 选择器
  ...
  // 动画效果,导航栏返回按钮
  currentPagesInstance.animate('.navbar .back', [{ color: '#fff' }, { color: '#000' }], 1000, {
    scrollSource: '#scroller',
    timeRange: 1000,
    startScrollOffset: 0,
    endScrollOffset: 50,
...
const detailData = ref({} as OrderResult)
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower">
    <template v-if="detailData">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        <!-- 待付款状态:展示去支付按钮和倒计时 -->
        <template v-if="detailData.orderState === OrderState.DaiFuKuan">
        ...
        <template v-else>
          <!-- 订单状态文字 -->
          <view class="status"> {{ orderStateList[detailData.orderState]?.text }} </view>
