...
<template>
  ...
  <scroll-view scroll-y class="viewport">
      ...
      <view class="preview">
        <swiper circular @change="swiperChange">
          <swiper-item v-for="item in goodsData?.mainPictures" :key="item">
            <image mode="aspectFill" @tap="onTagImg(item)" :src="item" />
          </swiper-item>
        </swiper>
    ...
    <view class="detail panel">
      ...
      <view class="content">
        ...
        <!-- 图片详情 -->
        <image
          v-for="pic in goodsData?.details.pictures"
          :key="pic"
          mode="widthFix"
          :src="pic"
        ></image>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    <view class="icons">
      ...
      <navigator class="icons-button" url="/pages/cart/cart2?t=1">
